...
def screen_file(record, sectors, cache):
    ...
    with rasterio.open(path) as raster, h5py.File(path, 'r') as h:
        ...
        for row, col in indices:
            ...
            sector = containing_sector(lon, lat, sectors)
            if sector is None:
    ...
    return {'survey_id': record['survey_id'], 'bag_url': url, 'bag_sha256': record['file_sha256'],
            ...
            'counts': counts, 'sectors': assigned}
...
def main():
    ...
    output = {'schema_version': 1, 'scope': 'california-original-vr-native-depth-review',
              ...
              'method': 'Original fine-resolution BAG refinement cells (<=4 m), MLLW and supplied product uncertainty. A grid is assigned to one browse sector by its center; boundary grids may straddle sectors.',
              ...
              'sectors': summarize_by_sector(rows, sectors), 'files': rows}

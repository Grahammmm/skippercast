...
def main():
    ...
    with urlopen(Request(request_url,headers={'Authorization':'Bearer '+os.environ['ACTIONS_ID_TOKEN_REQUEST_TOKEN']}),timeout=20) as response:
        token=json.load(response)['value']
    cursor=None;totals={k:0 for k in ('checked','changes','delivered','held','in_app')}
    for _ in range(100):
        request=Request(url,data=json.dumps({'cursor':cursor}).encode(),headers={'Authorization':'Bearer '+token,'Content-Type':'application/json'},method='POST')

...
For original **regular** BAGs, `scripts/triage_regular_habitat_outlines.py` uses the same research-only queue contract. Bind a source-identity and physical-size profile in `catalog/regular-habitat-triage-profiles.json`, a pinned native NOAA/USGS review summary, its matching displayed outlines and a fresh complete bounded ENC screen. The first two bindings cover Point Conception and Gaviota. The same closure auditor then checks the resulting shortlist; a zero-result queue is retained as an honest finding. None of these stages turns historical structure into a fishing mark, and every new geography needs a locally suitable meter-based projection.

For a closure-screened regular-grid shortlist, `scripts/fetch_shortlist_original_bags.py` retrieves only the exact SHA-256-pinned NOAA BAG files used by its outlines. `scripts/audit_regular_shortlist_terrain.py` reopens their original 1–2 m MLLW depth and product-uncertainty cells, requires qualified cells beneath every displayed patch, and measures relief and plane-detrended complexity. The physical grade is uncalibrated to catches and remains research-only. The monthly Point Conception run repeats this source check; another region needs its own reviewed BAG datum, resolution, source identity and footprint before reusing the stage.

...
- `usgs-csmp-map-blocks.json`, `usgs-csmp-map-metadata.json`: the separate official USGS CSMP catalog and original XML metadata, including Monterey Bay blocks absent from the first DS 781 index. These are source leads, not verified fishing grounds or converted MLLW depths.

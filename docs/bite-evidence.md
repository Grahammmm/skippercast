...
The app reads the public feed at runtime. If unavailable, it shows the bundled edition with an explicit saved-snapshot label and its original date. After 36 hours the feed cannot earn Moderate evidence. Individual freshness limits are six hours for buoys, twelve for surface radar, 72 for MUR, 96 for daily chlorophyll and 36 for model initialization. These are disclosed product gates, not a scientific guarantee of representativeness. Blank/failed sources do not become calm water or zero fish. Rule-page hashes are change signals only.

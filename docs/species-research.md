...
The conditions score is the lower component score and the lowest hourly value across the window. Missing critical fields, inconsistent sources, stale/unidentified runs, active marine alerts or a fog/thunderstorm signal prevent a score. If the only problem is material model disagreement, use the rougher forecast, label Low confidence and cap the conditions score at 7.9; such a day cannot qualify as 8+. Days four through seven are provisional. Current alerts do not establish that no future warnings will be issued.

...
Every guide sector now has a local **Weather & ocean** view at the current map center. The browser requests ECMWF IFS and NOAA GFS winds, plus ECMWF WAM and NOAA GFS Wave seas, through Open-Meteo. It displays the returned wind and wave grid coordinates, checks 7 a.m.–1 p.m. Pacific hourly coverage for each of the next seven days, and shows the highest wind, gust and combined sea in that window. Primary and secondary swell components are 10 a.m. GFS Wave samples; the WAM endpoint does not populate those component fields here. Missing hours produce a dash and a coverage warning. Model disagreement and gusts below sustained wind are flagged, not turned into a comfort score. Requests run only when the visitor opens Conditions and are cached in that browser for 30 minutes by rounded map position.

This coastwide model view has no harbor-entrance clearance, tide/current prediction, advisory synthesis, detailed route or validated catch score. It must not be presented as a safe-departure recommendation. The deeper mapped-area forecasts retain their separate regional pipelines. [Open-Meteo's marine API](https://open-meteo.com/en/docs/marine-weather-api) documents the models, grid and terms; its [free API terms](https://open-meteo.com/en/terms) limit that service to non-commercial use. The app needs a licensed service before commercial operation.

...
The hourly West Coast radar-current binding now reads the [NOAA NDBC HFRNet THREDDS service](https://dods.ndbc.noaa.gov/thredds/catalog/hfradar.html), as [NOAA IOOS directs users to do](https://ioos.noaa.gov/project/hf-radar/). A September 24 22:00 UTC Morro Bay test subset had 154 valid vectors of 414 requested regional cells after requiring at least two contributing radars. The product reports a nominal 1.4 m depth and actual grid spacing near 0.054° latitude by 0.062° longitude. These are near-surface vectors, not bottom, reef-scale or harbor-bar currents. Crescent City's same-time sample had zero valid vectors; its source must show missing rather than borrowing another region's cells.

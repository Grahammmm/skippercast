...
All eight Channel Islands now have map-focus controls. San Nicolas and San Clemente use their existing broad locality bounds and new requested offshore forecast samples; both retain military-access notices and have no promoted fishing recommendations. The first San Clemente request returned a land grid despite `cell_selection=sea`, so its sample was moved west. A separate four-model probe then returned sea cells, but the production feed must keep the actual returned grid coordinates visible. A requested point is not a new high-resolution model or access clearance. Anacapa uses [NWS PZZ655](https://api.weather.gov/zones/forecast/PZZ655). Western islands use [NWS PZZ673](https://forecast.weather.gov/MapClick.php?TextType=2&zoneid=PZZ673); the southern island reference context uses [NWS PZZ676](https://forecast.weather.gov/MapClick.php?TextType=2&zoneid=PZZ676). Mainland tide stations remain explicitly distant references.

...
The browser checks the tile's SHA-256, region and target identity. It renders one image on demand, avoiding the cost of loading all seabed tiles on a phone. The relief view draws a 4 m mesh with **2× vertical scale**; the north-up image retains 2 m drawing cells. The caption identifies native resolution, survey year, measured coverage and datum. Unmeasured cells remain blank. The target ring is centered on the nearest native cell; the stored subcell offset is under a cell width.

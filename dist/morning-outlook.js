...
export function renderOutlook(rows, point) {
  ...
  banner.innerHTML = best
    ? `<span>${best.confidence === "Low" ? "Tentative best" : "Best conditions"} · ${local(best.time, { weekday: "short" })}${best.confidence === "Low" ? " · low confidence" : best.provisional ? " · provisional" : ""}</span><strong>${num(best.conditions)}/10 <small>· bite unknown</small></strong>`

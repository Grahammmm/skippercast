...
import {initIntelligence} from './intelligence.js?v=8.14';
import {initHabitatDynamics} from './habitat-map.js?v=8.14';
import {terrainSource,terrainMetricsHTML} from './terrain-evidence.js?v=8.14';
let tripExport;
...
import { initProtectedAreas } from "./protected-areas.js?v=8.14";
import { initDriftGuides } from "./drift-guides.js?v=8.14";
import { initCommercialAIS } from "./commercial-ais.js?v=8.14";
  ...
  for (const group of cells.values()) {
    if (group.length > 1) {
      ...
      const m = L.marker([t.latitude, t.longitude], {
        ...
        .bindTooltip(`${escapeHTML(t.label)} · ${t.center_depth_ft} ft · ${escapeHTML(selectedSpeciesFit(t,$('species-select').value)?.description||'fit unknown')}`)
...
function selectTarget(id, pan = true) {
  ...
  const fitHTML=`<div class="detail-section"><h3>Species habitat fit · 1–3</h3><p>1 = strongest mapped habitat fit (index ≥0.90 and ≥70% rough cover); 2 = intermediate (index ≥0.70); 3 = lower. The index compares mapped terrain only, not fish presence or catch odds. Survey interpretation confidence is separate.</p>${fits.map(({name,fit})=>`<p><strong>${name === 'lingcod' ? 'Lingcod' : 'Rockfish'} · ${fit.rank}/3</strong> · terrain index ${fit.score.toFixed(2)} — ${escapeHTML(fit.reason)}</p>`).join('')}</div>`;
  $("detail").innerHTML =

...
export function renderBottom(canvas, data, mode = "relief", options={}) {
  ...
  if (mode === "plan") {
    ...
    ctx.fillStyle="#fff";ctx.font="16px sans-serif";ctx.fillText("N",62+39*Math.cos(a)-5,62+39*Math.sin(a)+5);
  }
...
export async function mountBottom(container, target) {
  ...
  try {
    ...
    fields.innerHTML=`<label>Rotate relief <input data-bottom="azimuth" type="range" min="0" max="360" step="5" value="0"></label><label>Section bearing · ° true<input data-bottom="section" type="range" min="0" max="179" step="1" value="90"></label><label>Highlight from · ft<input data-bottom="min" type="number" min="0" max="1500" value="${Math.floor(data.depth_range_ft[0])}"></label><label>Highlight to · ft<input data-bottom="max" type="number" min="0" max="1500" value="${Math.ceil(data.depth_range_ft[1])}"></label><label><input data-bottom="band" type="checkbox">Highlight this depth band</label>`;
    ...
    const draw=()=>{
      ...
      caption.textContent=`USGS ${data.survey_year} · ${data.native_cell_m} m native cells · ${Math.round(data.coverage_fraction*100)}% measured. ${mode==='relief'?'Relief: 2× vertical scale; 4 m drawing mesh.':'North-up depth: 2 m drawing cells; orange section line.'} White ring: target. Highlighting does not qualify a fishing area; this window may extend beyond the screened footprint. No individual boulder or fish inference.`;
      ...
      if(!valid.length){c.fillStyle='#e5f3f4';c.font='16px sans-serif';c.fillText('No measured cells on this section.',30,100);profile.setAttribute('aria-label','No measured cells on this section.');return;}
      ...
      c.fillStyle='#e5f3f4';c.font='15px sans-serif';c.fillText(`${lo.toFixed(1)} ft`,5,25);c.fillText(`${hi.toFixed(1)} ft`,5,162);c.fillText(`0 → ${Math.round(series.at(-1).distance_ft)} ft along ${section}° / ${section+180}° section · gaps remain blank`,50,189);
      profile.setAttribute('aria-label',`${section} degree section, ${Math.round(series.at(-1).distance_ft)} feet long, depths ${lo.toFixed(1)} to ${hi.toFixed(1)} feet MLLW. Independent depth axis scale.`);
    };
    ...
    tools.append(fields,profile);const note=document.createElement('p');note.className='small';note.textContent='The section uses nearest native survey cells. Its depth axis is scaled independently for readability; slopes in this graph are not to scale.';tools.append(note);
    ...
    status.remove();figure.append(controls,canvas,caption,tools);draw();

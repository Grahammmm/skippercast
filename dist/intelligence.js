import {getRegion,localContext} from './region.js?v=8.4';
import {esc,num,from,local} from './marine-charts.js?v=8.4';
import {readConditions,angleBetween,distanceNm} from './marine-data.js?v=8.4';
...
function verificationHTML(data) {
  ...
  return `<p><strong>${esc(v.status)}</strong> · ${v.archived_forecasts.toLocaleString()} prospective forecast samples archived · ${v.matched_samples} matched observations.</p>${v.groups.length?`<div class="matrix-scroll"><table class="forecast-matrix"><thead><tr><th>Model / station</th><th>Lead</th><th>Mean absolute error</th><th>Bias</th><th>Samples / days</th></tr></thead><tbody>${v.groups.map(g=>`<tr><th>${esc(g.model)} · ${esc(g.station)}<small>${esc(g.variable)}</small></th><td>${g.lead_hours.join('–')} h</td><td>${num(g.mae)} ${g.unit}</td><td>${g.bias>0?'+':''}${num(g.bias)} ${g.unit}</td><td>${g.n} / ${g.distinct_days} · ${esc(g.status)}</td></tr>`).join('')}</tbody></table></div>`:'<p>The archive has started. Results appear as forecasts reach their valid time and matching buoy observations arrive. Historical forecasts are not reconstructed after the fact.</p>'}<p class="small">Positive bias means the model ran high. At least 30 matches across 7 days are required for a descriptive report; even then, no automatic correction is applied. ${esc(v.limitations)}</p>`;
}

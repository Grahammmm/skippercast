...
export function detailHTML({
  ...
  const waveId = family === "gfs" ? "ncep_gfswave016" : "ecmwf_wam",
  ...
  return `${requested ? `<p class="small">Selected ${esc(requested.label || "map location")}: nearest requested forecast sample is ${num(distanceNm(requested, p))} nm away. Values are regional, not reef-scale.</p>` : ""}
    ...
    <section class="marine-card"><h3>How we compare forecast models</h3><p>For the next 72 hours, compare both models hour by hour with current buoy readings and the NWS marine forecast. For days 4–7, treat the outlook as provisional and watch whether the independent wind and wave models agree. Ratings use the rougher available conditions rather than choosing the calmer forecast.</p><p class="small">SkipperCast archives forecasts before their valid time and compares them with later observations by station, variable and lead time. There is not yet enough local evidence to declare one model the winner for days 1–3 and another for days 4–7. Open-Meteo delivers the model data; it is not another independent model. See the Guide’s “How the forecasts perform locally” panel for actual sample counts and errors.</p></section>

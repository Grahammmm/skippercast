...
export function detailHTML({
  ...
  return `${requested ? `<p class="small">Selected ${esc(requested.label || "map location")}: nearest requested forecast sample is ${num(distanceNm(requested, p))} nm away. Values are regional, not reef-scale.</p>` : ""}
    ...
    </div><div data-condition-panel="tides" hidden><section class="marine-card tide-card"><div class="section-title"><div><div class="eyebrow">NOAA · ${esc(getRegion().stations.tide_name)} ${esc(getRegion().stations.tide)}</div><h3>Tides</h3></div><strong>${num(tideAt(bundle.tides, t))} ft <small>MLLW</small></strong></div>${lineChart(tide, { label: getRegion().stations.tide_name + " predicted tide", color: "#775eab", selected: t, unit: "ft", min: 0 })}<div class="tide-extremes">${extreme.map((x) => `<span><strong>${x.type === "H" ? "High" : "Low"} ${num(x.height)} ft</strong>${local(x.time, { weekday: "short", hour: "numeric", minute: "2-digit" })}</span>`).join("")}</div><p class="small">Astronomical tide prediction. ${esc(getRegion().stations.tide_note)} High/low water does not establish slack current.</p><p class="observation">${waterFresh ? `Latest observed water level: <strong>${num(Number(water.v))} ft MLLW</strong> · ${full(waterEpoch)} (NOAA preliminary). This observation stays at its recorded time as you scrub.` : "Latest water-level observation unavailable or older than two hours; predicted tide remains separate."}</p><a href="https://tidesandcurrents.noaa.gov/noaatidepredictions.html?id=${getRegion().stations.tide}" target="_blank" rel="noopener">NOAA tide station ↗</a></section>
    ...
    <section class="marine-card"><h3>Before you go</h3><p>Inspect the full trip, including the return. A pleasant hour at a fishing spot does not establish a comfortable passage or safe entrance.</p><div class="forecast-links"><a href="https://forecast.weather.gov/MapClick.php?TextType=2&zoneid=${getRegion().marine_zones.coastal}" target="_blank" rel="noopener">NWS coastal forecast ↗</a><a href="https://forecast.weather.gov/MapClick.php?TextType=2&zoneid=${getRegion().marine_zones.offshore}" target="_blank" rel="noopener">NWS offshore forecast ↗</a><a href="https://www.ndbc.noaa.gov/station_page.php?station=${getRegion().stations.nearshore_buoy}" target="_blank" rel="noopener">${esc(getRegion().stations.nearshore_buoy_name)} reference buoy ↗</a><a href="${getRegion().harbor.information_url}" target="_blank" rel="noopener">${esc(getRegion().harbor.name)} Harbor ↗</a><a href="https://wildlife.ca.gov/Fishing/Ocean/Regulations/Fishing-Map/Central" target="_blank" rel="noopener">Current fishing rules ↗</a></div><p class="small">Daily buoy samples appear in the recent fishing-evidence panel. Live entrance conditions are not loaded here. Active-alert checks do not cover unissued warnings later in the forecast.</p></section>

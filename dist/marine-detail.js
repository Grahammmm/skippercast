...
export function detailHTML({
  ...
  return `${requested ? `<p class="small">Selected ${esc(requested.label || "map location")}: nearest requested forecast sample is ${num(distanceNm(requested, p))} nm away. Values are regional, not reef-scale.</p>` : ""}
    <div class="conditions-hero"><div><span class="eyebrow">COMBINED SIGNIFICANT SEAS</span><strong class="sea-number">${num(c.sea.height)}<small> ft</small></strong><span>${num(c.sea.period)} s ${family === "gfs" ? "primary-wave" : "mean"} period · from ${from(c.sea.from)}</span></div><div class="hero-wind"><span>${weatherName(c.weatherCode)}</span><strong>${num(c.wind, 0)}<small> kt wind</small></strong><span>Gust ${num(c.gust, 0)} kt · from ${from(c.windFrom)}</span><span>${num(c.air, 0)}°F air · ${c.visibility === null ? "Visibility unavailable" : `${num(c.visibility / 1609.344)} mi visibility`}</span><span>${num(c.rain)} mm precipitation</span></div></div>
    <div class="comfort-explainer ${status.level}"><strong>${status.label}${provisional ? " · provisional" : ""}</strong><p>${status.flags.map(esc).join(". ")}.</p><p class="small">Preferred screen: wind ≤8 kt, gusts ≤12 kt, combined seas ≤3 ft, chop ≤1 ft. Wind, gust, and combined-height thresholds use the rougher of both models. It is not a 9/10 trip rating or a harbor clearance. ${p.offshore ? "Check the coastal entrance and every offshore return hour." : "Tide height alone cannot determine bar safety or slack current."}</p></div>
    <div class="ocean-detail-grid"><section class="marine-card wave-card"><div class="eyebrow">UNDERSTAND THE MOTION</div><h3>What makes up the sea</h3>${family === "ecmwf" ? '<p class="small">This ECMWF feed provides combined seas. Select NOAA GFS above for available swell and chop components.</p>' : ""}${waveSketch(c.swell, "Primary swell", "#198f9e")}${waveSketch(c.secondary, "Secondary swell", "#ad5c85")}${waveSketch(c.chop, "Wind chop", "#cb883e")}<p class="small">Schematic: height changes amplitude; period changes spacing. Period is seconds per cycle, not a countdown for individual waves. ${short ? "Short chop may make the ride sharp. " : ""}${crossed ? "Crossing swell directions may increase roll. " : ""}Combined significant height is not the sum of component heights; larger individual waves occur. GFS supplies a primary-wave period and direction; ECMWF supplies mean values, so those headline periods are not like-for-like.</p></section><section class="marine-card"><div class="eyebrow">NORTH UP</div><h3>Direction &amp; approach</h3>${rose(c)}<label>Boat heading (true)<input id="boat-heading" type="number" min="0" max="359" step="1" value="${Number.isFinite(heading) ? heading : ""}" inputmode="numeric"/></label><p id="encounter-note" class="small"></p><p class="small">Arrows travel toward; source directions are FROM. Actual encounter frequency also depends on speed, depth, and wave length. This is not a vessel-motion simulation.</p></section></div>
    <section class="marine-card"><div class="section-title"><div><div class="eyebrow">24-HOUR CONTEXT · ${day(begin)}</div><h3>Wind &amp; sea trend</h3></div><span>${family === "gfs" ? "NOAA GFS" : "ECMWF"}</span></div><div class="trend-label">Combined seas (ft)</div>${lineChart(
    ...
    <section class="marine-card tide-card"><div class="section-title"><div><div class="eyebrow">NOAA · PORT SAN LUIS 9412110</div><h3>Tides</h3></div><strong>${num(tideAt(bundle.tides, t))} ft <small>MLLW</small></strong></div>${lineChart(tide, { label: "Port San Luis predicted tide", color: "#775eab", selected: t, unit: "ft", min: 0 })}<div class="tide-extremes">${extreme.map((x) => `<span><strong>${x.type === "H" ? "High" : "Low"} ${num(x.height)} ft</strong>${local(x.time, { weekday: "short", hour: "numeric", minute: "2-digit" })}</span>`).join("")}</div><p class="small">Astronomical tide prediction at a nearby station. This is not Morro Bay bar current, slack water, or an entrance clearance.</p><p class="observation">${waterFresh ? `Latest observed water level: <strong>${num(Number(water.v))} ft MLLW</strong> · ${full(waterEpoch)} (NOAA preliminary). This observation stays at its recorded time as you scrub.` : "Latest water-level observation unavailable or older than two hours; predicted tide remains separate."}</p><a href="https://tidesandcurrents.noaa.gov/noaatidepredictions.html?id=9412110" target="_blank" rel="noopener">NOAA tide station ↗</a></section>
    <div class="ocean-detail-grid"><section class="marine-card"><div class="eyebrow">INDEPENDENT MODEL CHECK</div><h3>${family === "gfs" ? "ECMWF comparison" : "NOAA GFS comparison"}</h3><p><strong>${num(other.wind)} / ${num(other.gust)} kt</strong> wind / gust<br><strong>${num(other.sea.height)} ft @ ${num(other.sea.period)} s</strong> combined seas<br>Chop ${num(other.chop.height)} ft @ ${num(other.chop.period)} s</p><p class="small">${family === "gfs" ? "ECMWF headline period is a mean; GFS is primary-wave period." : "GFS headline period is primary-wave; ECMWF is a mean."} Same requested location and hour; each model uses its own grid. Disagreement is uncertainty, not a reason to select the calmer model.</p></section><section class="marine-card"><div class="eyebrow">OCEAN CONTEXT</div><h3>Temperature &amp; surface current</h3><p><strong>${num(c.sst)}°F</strong> sea surface<br><strong>${num(c.current)} kt</strong> surface current toward ${from(c.currentTo)}</p><p class="small">Météo-France / Copernicus · about 8 km. Modeled surface temperature is not bottom temperature or satellite SST. Surface current is not bottom drift or a usable harbor-current prediction.</p></section></div>
    <section class="marine-card"><h3>Before you go</h3><p>Inspect the full trip, including the return. A pleasant hour at a fishing spot does not establish a comfortable passage or safe entrance.</p><div class="forecast-links"><a href="https://forecast.weather.gov/MapClick.php?TextType=2&zoneid=PZZ645" target="_blank" rel="noopener">NWS coastal forecast ↗</a><a href="https://forecast.weather.gov/MapClick.php?TextType=2&zoneid=PZZ670" target="_blank" rel="noopener">NWS offshore forecast ↗</a><a href="https://www.ndbc.noaa.gov/station_page.php?station=46215" target="_blank" rel="noopener">Live Diablo Canyon buoy ↗</a><a href="https://www.morrobayca.gov/133/Harbor" target="_blank" rel="noopener">Morro Bay Harbor ↗</a><a href="https://wildlife.ca.gov/Fishing/Ocean/Regulations/Fishing-Map/Central" target="_blank" rel="noopener">Current fishing rules ↗</a></div><p class="small">Buoy observations and live entrance conditions are not loaded in this page. Active-alert checks do not cover unissued warnings later in the forecast.</p></section>
